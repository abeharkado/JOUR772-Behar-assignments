...
We'll be using a service called Groq for the examples here. You should [sign up for a f1ree account](https://www.groq.com) and [create an API key](https://console.groq.com/docs/quickstart). Make sure you copy that key. We'll also need to install an R package to handle the responses:
...
```{r}
filter_response <- axolotr::ask(
  prompt = "Explain how to filter data with multiple variables",
  model = "llama-3.1-8b-instant"
)
...
Personally, i find the answer just a bit hard to decipher because of the formatting. While, from what I can tell, the response was accurate and concise in its explanation, I think I prefer to ask for help from a person, not AI.
