...
**Answer:** It does make sense to me because it takes the total of expenses and groups them by state. We told it to find the state in the text under address and form a new column for it and then asked it to group our data by that parameter. I do find that the different expenses for each state are a little odd since I would assume California or Florida to have more expenditures than Maryland for example but that is something we could dig a little deeper on.
...
1.  How much is each state spending in a month or a determined amount of time?

2.  How much does each state spend on candidate committees?

3.  Follow up on question 2: if there is a difference in spending per candidate, is there an influence on who won that election?
