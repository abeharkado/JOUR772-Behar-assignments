...
**A1. The story here is that Moore did significantly better overall in election results than Brown did. The county with the most interesting results would be Prince George's County and Baltimore City since they were the only counties where Brown did better.**
...
**A2. The potential story here is why Talbot County is the only outlier in the datset. Why is the rate here so high? I think if we had access to other months' outage data you would have to see if there is a trend over time.**
...
md_counties_fixed <- md_counties_fixed|> mutate (NAME = case_when
...
**A3. Caroline county had the highest increase in EVs and about double the amount as the Somerset which was second in that list. One question I have is if we could compare this with median income in that county, would there be a correlation? Howard and Montgomery county stand out in this data and it differs quite a bit from the previous map.**
...
md_evs <- md_counties |> inner_join(ev, by = c('NAME' = 'county'))
...
md_evs_geometry <- md_evs |> inner_join(md_county_population, by = 'GEOID')
...
md_evs_geometry<- md_evs_geometry|>
...
ggplot()+
  geom_sf(data=md_evs_geometry, aes(fill= evs_per_cap))+
  theme_minimal()+
  scale_fill_binned()
```
