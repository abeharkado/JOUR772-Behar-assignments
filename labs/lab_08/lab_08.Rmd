...
**A1. The county with the highest percentage of total absentee ballots returned is Worcester.**
...
ballot <- md_absentee_ballots |> 
  mutate(pct_dems = (dem_received/total_received)*100)|>
  mutate(pct_reps = (rep_received/total_received)*100)|>
  mutate(pct_diff = (pct_dems - pct_reps))|>
  arrange(pct_dems)|>
  head(10)
...
ballot |>
  ...
  coord_flip() + 
  labs(
    title="Top 10 counties with Fewest Democratic Ballots Returned",
    x = "County",
    y = "Percent Difference of Returned Democratic and Republican Ballots",
    caption = "source: Maryland State Board of Elections"
  )
```
...
**A3. For Frederick County, Democrats overwhelmingly returned their absentee ballots compared to Republicans and Others. Yes, it does match what we know about absentee voting habits.**

```{r}
dems_fred <- md_absentee_ballots_date |>
  filter(county_name == "FREDERICK")

dems_fred |>
  ggplot() + 
  geom_line(aes(x=date, y= total_received, color= "Total"))+
...
**A4. Montgomery County seems to be over-performing in relation to the overall trend. The main points of the chart seem to be on the lower side with only the Democrat data extending past the other parties.**
...
md_absentee_ballots|>
  ...
  geom_point(aes(x=oth_sent, y= oth_received, color= "Other"))+
  ...
  geom_text_repel(aes(x = oth_sent, y = oth_received, label = county_name),
            color = "black", size=2)+
  geom_smooth(aes(x = dem_sent, y = dem_received),method = lm, se= FALSE, color= "orange")+
...
md_absentee_ballots|>
  ...
  geom_text(aes(x=rep_sent, y=rep_receiyved, label = county_name), check_overlap = TRUE)+
  geom_smooth(method = lm, aes(x = rep_sent, y = rep_received))+
  labs(
    title="Anne ARundel Leads GOP in Returned Ballots",
    subtitile= "Montgomery County Republicans trail behind other counties",
    x="ballots sent",
    y="ballots returned",
    caption= "source: Maryland State Board of Elections"
  )
