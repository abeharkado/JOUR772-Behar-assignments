...
allegany<- read_csv("data/allegany_911.csv")|> clean_names()
carroll<- read_csv("data/carroll_911.csv")|> clean_names()
cecil <- read_csv("data/cecil_911.csv")|> clean_names()

county_combined <- bind_rows(list(allegany, carroll, cecil))
...
baltimore<- read_csv("data/baltimore_911.csv")|> clean_names()
md_zcta <- read_csv("data/maryland_zcta.csv")|>clean_names() |> rename(zip_code = zcta5n)



baltimore_and_demographics <- baltimore |> left_join(md_zcta, join_by(zip_code))
...
A1. Carroll had the most calls (448), followed by Cecil (438), and finally Allegany (418).
...
A2. Except for Allegany in October and Carroll in March, the summer months seem to be when most calls come in. The location with the most calls is 314 GROVE NECK RD in Cecil County, which turns out to be an inpatient rehab facility.
...
A3. The zip code with the most calls was 21217 with 774 calls and only 7 observations fit that criteria. The zipcode with the highest rate of calls per 1000 people is also 2127 and the neighborhoods Penn North, Sandtown-Winchester and Upton are the ones with the most calls.

```{r}
under_18 <- baltimore_and_demographics |> 
  filter(pct_under_18 >= 75)

baltimore_and_demographics |> 
  group_by(zip_code, population) |>
  summarize (total = n()) |>
  mutate(per_cap = (total/population)*1000)|>
...
A4:Is there a difference in numbers of 911 calls made from predominantly white versus predominantly black zipcodes? Yes, there is. Predominantly black zipcodes in Baltimore experience about 3,000 more calls.
