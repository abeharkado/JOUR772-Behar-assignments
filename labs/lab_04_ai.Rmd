...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "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")
```
...
maggie_response <- axolotr::ask(
  prompt = paste("Given the following text, extract relevant information into a CSV file with the following structure, including these variable headers, with no yapping and do not include the first sentence of the response: title, year, role, director, co-stars, awards", text),
  model = "llama-3.1-8b-instant"
)

maggie_response

maggie_response_df <- read_csv(maggie_response)
```

**Answer: The information is correct and it does seem like the information came from the article. I think there are some things missing that Groq didn't find though.**
...
```{r}
#Add code below to talk to Groq and display its response
course_response <- axolotr::ask(
  prompt = paste("can you explain what the r_code_example does to the umd_courses csv file to a 5 year old", r_code_example),
  model = "llama-3.1-8b-instant"
)
...
lab_response <- axolotr::ask(
  prompt = paste("tell me simply if this code your_r_code correct",your_r_code),
  model = "llama-3.1-8b-instant"
)
...
**Answer: I think Groq's answer is interesting. I like that it included a possible improvement to the code I have but I think it is till a bit hard to understand.**
...
brainstorm_response <- axolotr::ask(
  prompt = paste("Based on a dataset about car crash incidents in Montgomery County in Maryland that details the time and date of the incident, if it was a hit and run, the exact location and the damages give me possible news story ideas"),
  model = "llama-3.1-8b-instant"
)
...
**Answer: I think it went a little further in some ideas than what I described was in the dataset but they seem feasible. I like how it provides a small description on each idea.**
...
news_article <- axolotr::ask(
  prompt = paste("based on brainstorm_response, write a 500 word news article about a car crash at the intersection between Knox Road and Regents Drive",brainstorm_response),
  model = "llama-3.1-8b-instant"
)
...
**Answer: The main issue I think is that it made up a story about a crash in that intersection. Though it sounds coherent and follows a plausible series of events, the details are made up which makes the story false. It even includes fictional quotes!**
