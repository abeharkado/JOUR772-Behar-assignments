...
**A1: the file name is actually a URL.**
...
**A2: I think the impact of the earthquake (magnitude), the time and date and the place would be important if we wanted to map out these earthquakes and see how frequent/destructive they are.**
...
**A3: The depth for the deepest earthquake is 628.362 km and its magnitude was 4.60.**
...
**A4: You can use a filter to find the earthquakes that have a magnitude larger than 6 and there are 6 observations.**
...
**A5: You can add another filter to the same data. There are 4 observations.**
...
**A6: You can change the and in the function to or to get back 7474 observations that fit either these criteria.**
...
**A7: You could filter with string detect the place column for Alaska. 2168 observations come up.**
...
**A8: The first two most common types of tremors are earthquakes and quarry blasts.**
...
earthquakes |>
  group_by(type) |>
  summarize (total = n(), average_depth = mean(depth))
```

**A9: The average depth of the earthquake type is 19.4 (rounded to the nearest tenth). It is surprising to have negative numbers for the explosion and quarry blast and to have an average of 0 for the landslide.**
...
**A11: you can mutate into a new column putting a case_when function for any mag that is above 3 to come out as 'serious' and default the rest to 'not serious'.**
...
What network reports the most earthquakes?
...
```{Are there observations that have a mag error larger than 0.5?}
